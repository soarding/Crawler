...
var DB_CONN_STR = 'mongodb://localhost:27017/ding';    
			      ...
			      //1、收集用户个人信息，昵称、园龄、粉丝、关注
						...
						var currentBlogApp = url.split('/p/')[0].split('/')[3],	
							requestId = url.split('/p/')[1].split('.')[0];
                                                ...
                                                var art={};
                                                art.currentBlogApp=currentBlogApp;
                                                art.requestId =requestId;
                                                var strs=$('title').text().split(' - ');
                                                var author=strs[strs.length-2];
                                                art.author=author;
                                                art.title= $('title').text().split(' - '+author+' - ')[0];
                                                articleData.push(art);
...
var collection = db.collection('tb1');
   for(var i=0 ; i<articleData.length ; i++){
			  	var eachDate = JSON.stringify(articleData[i]),
			  		eachDateJson = articleData[i];

collection.insert(eachDateJson, function(err, result) { 
        if(err)
        {
...
var collection = db.collection('tb');
   for(var i=0 ; i<catchDate.length ; i++){
			  	var eachDate = JSON.stringify(catchDate[i]),
			  		eachDateJson = catchDate[i];
